...
void NeedlePlanner::compute_tissue_frame_wrt_camera(Eigen::Vector3d entrance_pt,
    ...
    // DLC for rotate counter clock put -1 (exit_pt-entrance_pt)  
    ...
    outfile.open ("kt_trajectory.csp", std::ofstream::out | std::ofstream::app);   
        ...
        {  
           ROS_WARN("ik ok"); 
            ...
            //kt_p1  = des_kt_gripper1_wrt_base.translation();
            //kt_r1n = des_kt_gripper1_wrt_base.linear().col(0);
            //kt_r1b = des_kt_gripper1_wrt_base.linear().col(2); 
...
void NeedlePlanner::compute_needle_drive_gripper_affines(vector <Eigen::Affine3d> &gripper_affines_wrt_camera, Eigen::VectorXi &ik_ok_array, int &ik_score) {
    ...
    for (int ipose=0;ipose<45;ipose++) {
        ...
        if(debug_needle_print) ROS_INFO("des_gripper1_wrt_base");
        if(debug_needle_print) print_affine(des_gripper1_wrt_base);

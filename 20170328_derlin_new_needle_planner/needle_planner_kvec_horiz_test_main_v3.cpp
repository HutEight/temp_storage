...
int main(int argc, char** argv) {
    ...
    Eigen::VectorXi ik_ok_array(45); //Add by DLC 5-26-2016 also in test_main, test_main_v2
    int ik_score=0;
    ...
    v_entrance_to_exit0 << 0, -1, 0; // DLC corresponds to chosen needle kvec along 1,0,0, needle nagative rotation * for special case only.
    
    //v_entrance_to_exit0 << 0, 1, 0; // DLC, needle positive rotatation right hand rule * special case only 
           ...
           for (needle_x = 0; needle_x < 0.1; needle_x += 0.7854){   
            ...
            for (kvec_yaw = 1.5708; kvec_yaw < 1.5709; kvec_yaw += 0.1) {
                ...
                // needlePlanner.compute_needle_drive_gripper_affines is computing circular needle driving
                  ...
                  needlePlanner.compute_needle_drive_gripper_affines(gripper_affines_wrt_camera, ik_ok_array, ik_score);
